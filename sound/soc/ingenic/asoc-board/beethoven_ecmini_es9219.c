...
static struct snd_switch_data {
	...
	extern unsigned char linein_init_switch_data;
#else
	int akm4951_i2c_write_regs(unsigned char reg, unsigned char* data, unsigned int len){return 0;}
	int akm4951_i2c_read_reg(unsigned char reg, unsigned char* data, unsigned int len){return 0;}
	unsigned char linein_init_switch_data;
#endif
...
#endif
                linein_init_switch_data = 0x04;

                data = 0x0a; //lin3
                akm4951_i2c_write_regs(0x03, &data, 1);

                printk("=====> line-in plugin <=====\n");
        }else{
                state = 0;
#if 0
                gpio_direction_output(GPIO_FM_AUX_SEL, GPIO_FM_AUX_SEL_FM);
#endif
                linein_init_switch_data = 0x02;

                data = 0x05; //lin2
                akm4951_i2c_write_regs(0x03, &data, 1);

                printk("=====> line-in plugout <=====\n");
        }
...
int i2s_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params) {
	...
	printk("es9118 platform i2s_hw_params\n");
	printk("channels:%d fmt_width:%d sample_rate:%d\n", channels, fmt_width, sample_rate);
...
static struct snd_soc_dai_link soc_dais[] = {
    [0] = {
		...
		//beethoven akm4951
        .name = "beethoven es9219",
        .stream_name = "beethoven es9219",
        .platform_name = "jz-asoc-aic-dma",
        .cpu_dai_name = "jz-asoc-aic-i2s",
        .init = i2s_dai_link_init,
        //.codec_dai_name = "akm4951-dai",
        .codec_dai_name = "es9219-dai",
        .codec_name = "es9219_1.1-0048",
        .ops = &beethoven_i2s_ops,
    },
   [1] = {
        .name = "ecminithird PCMBT",
        .stream_name = "ecminithird PCMBT",
...
static int snd_beethoven_probe(struct platform_device *pdev) {
	...
	printk("snd_beethoven_probe es9118 soc\n");
