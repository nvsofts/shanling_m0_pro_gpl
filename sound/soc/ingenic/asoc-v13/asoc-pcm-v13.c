...
#define JZ_PCM_RATES   (SNDRV_PCM_RATE_8000_705600&(~SNDRV_PCM_RATE_64000))

#ifdef CONFIG_PRODUCT_X1000_M5S
extern int ak4493_power;
#elif defined(CONFIG_PRODUCT_X1000_M2X)
extern int ak4490_power;
#elif defined(CONFIG_PRODUCT_X1000_H5)
extern unsigned char dual_ak4493_power;
#endif
...
static struct pcm_conf_info pcm_conf_info = {
		...
		.pcm_imsb_pos = COINCIDENT_MODE,
		.pcm_omsb_pos = COINCIDENT_MODE,
		.pcm_sync_len = 1,
		.pcm_slot_num = 2,
};
		...
		struct snd_soc_dai *dai)
{
	...
	if (!jz_pcm->pcm_mode) {
			...
			if (pcm_conf_info.pcm_device_mode == PCM_SLAVE) {
				...
				printk("=====pcm master no implement====\n");
			}
			...
			__pcm_set_receive_trigger(dev, 8);
...
static int jz_set_pcm_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	return 0;
}
...
static int jz_set_pcm_sysclk(struct snd_soc_dai *dai, int clk_id,
...
static struct snd_soc_dai_ops jz_pcm_dai_ops = {
	...
	.set_fmt    = jz_set_pcm_fmt,
	.set_sysclk = jz_set_pcm_sysclk,
