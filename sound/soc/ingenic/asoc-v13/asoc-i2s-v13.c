...
#define JZ_I2S_RATE (SNDRV_PCM_RATE_8000_192000 | SNDRV_PCM_RATE_352800)
	
//#define CONFIG_JZ_AIC_MASTER	1
...
static int jz_i2s_set_rate(struct device *aic ,struct jz_aic* jz_aic, unsigned long sample_rate)
{
	...
	printk("jz_i2s_set_rate 0 sysclk:%d sample_rate:%d\n", sysclk, sample_rate);
	ret = clk_set_rate(jz_aic->clk, sysclk);
	printk("jz_i2s_set_rate 1\n");
				...
				struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	I2S_DEBUG_MSG("enter %s, substream = %s\n", __func__,
		      (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) ? "playback" : "capture");
	I2S_DEBUG_MSG("channels:%d fmt_width:%d sample_rate:%d\n", channels, fmt_width, sample_rate);
	I2S_DEBUG_MSG("(1 << params_format(params)):%d  JZ_I2S_FORMATS:%d\n",(1 << params_format(params)), JZ_I2S_FORMATS);
...
static int jz_i2s_platfrom_probe(struct platform_device *pdev)
{
	...
	jz_aic = dev_get_drvdata(jz_i2s->aic);
	...
	if (IS_ERR_OR_NULL(jz_aic->clk)) 
	{
		...
		jz_aic->clk = NULL;
		dev_err(&pdev->dev, "Failed to get  cgu_i2s clock: %d\n", ret);
		return ret;
	}
