...
static int hyn_get_ic_information(struct hyn_ts_data *ts_data)
{
    ...
    printk("chip_ID = 0x%02x, FW_ver = 0x%02x\n",chip_id[0], chip_id[1]);
    return 0;
}
...
int hyn_ctpm_fw_upgrade_with_i_file(void)
{
	...
	printk("======chipchecksum[0x%04X]=====checksum[0x%04X]===\n",chipchecksum,checksum);
        ...
        if (EVENT_DOWN(events[i].flag)) {
			...
			if(i == 0)
			{
		       if (events[i].area <= 0) {
				...
				input_report_abs(data->input_dev, ABS_X,HYN_Y_MAX_DISPLAY_DEFAULT-events[i].y);
				input_report_abs(data->input_dev, ABS_Y,events[i].x);
				//input_report_abs(data->input_dev, ABS_PRESSURE, events[i].p);
				input_report_abs(data->input_dev, ABS_PRESSURE,0x01);
				//printk("=========x[%d]===y[%d]==[%d]==\n",HYN_Y_MAX_DISPLAY_DEFAULT-events[i].y,events[i].x,events[i].p);
			}
			#else
            input_report_abs(data->input_dev, ABS_MT_TRACKING_ID, events[i].id);
#if HYN_REPORT_PRESSURE_EN
...
static int hyn_parse_dt(struct hyn_ts_platform_data *pdata)
{
    ...
    pdata->x_max = HYN_Y_MAX_DISPLAY_DEFAULT; //xy change
    pdata->y_max = HYN_X_MAX_DISPLAY_DEFAULT; //xy change
