...
#define FST_ADDR 0x15

static struct i2c_board_info hi_info = {
     I2C_BOARD_INFO("i2c-cst8xx", FST_ADDR),
};
...
int update_i2c_read(unsigned int reg_addr, unsigned int reg_addr_num, unsigned char *reg_data, unsigned int data_num)
{
    unsigned char tmp_buf0[4];
    //unsigned char tmp_buf1[4];
    int ret = 0;
...
int update_i2c_write(unsigned int reg_addr,unsigned int reg_addr_num, unsigned char *buf, unsigned int data_byte_num)
{
    int ret = 0;
    int idx = 0;
    unsigned char tmp_buf[8];
    ...
    if(data_byte_num > (8-reg_addr_num)){
        printk("\r\n[HYN]write len is overflow !!");
        return -1;
    }
