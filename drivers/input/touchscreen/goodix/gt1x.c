...
static const char *input_dev_phys = "input/ts";
	...
	int gt1x_int_gpio;
#else
	int gt1x_rst_gpio = GTP_RST_PORT;
	int gt1x_int_gpio = GTP_INT_PORT;
#endif
...
static irqreturn_t gt1x_ts_irq_handler(int irq, void *dev_id)
{
	...
	//printk("gt5663 have int add by zxj\n");
...
#else
	input_report_abs(input_dev, ABS_X, (u16)x);
	input_report_abs(input_dev, ABS_Y, (u16)y);
