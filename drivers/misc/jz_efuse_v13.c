...
#define CUT_ID_ADDR_SEG1            (0x220)
#define CUT_ID_ADDR_SEG2            (0x230)
#define CUT_ID_ADDR_SEG2_WORD0      (0x230)
#define CUT_ID_ADDR_SEG2_WORD1      (0x234)
#define CUT_ID_ADDR_SEG2_WORD2      (0x238)
#define CUT_ID_ADDR_SEG2_HALF_WORD3 (0x23c)
...
uint32_t seg_addr[] = {
	...
	PTR_ADDR,
    CUT_ID_ADDR_SEG1,
    CUT_ID_ADDR_SEG2_WORD0,
    CUT_ID_ADDR_SEG2_WORD1,
    CUT_ID_ADDR_SEG2_WORD2,
    CUT_ID_ADDR_SEG2_HALF_WORD3,
};
...
int jz_efuse_read(uint32_t seg_id, uint32_t data_length, uint32_t *buf)
{
    ...
    case CHIP_ID:
        if (bit_num != 128) {
            dev_err(efuse->dev, "read segment %d data must length %d = 128 bit",
                    seg_id, bit_num);
            return -1;
        }
        break;
    ...
    case USER_ID:
        if (bit_num != 256) { /* read must word align */
            dev_err(efuse->dev, "read segment %d data must length %d = 256 bit",
                    seg_id, bit_num);
            return -1;
        }
        break;
    case PROTECT_ID:
        if (bit_num > 32) { /* read must word align */
            dev_err(efuse->dev, "read segment %d data length %d > 32 bit",
                    seg_id, bit_num);
            return -1;
        }
        break;
...
static int jz_efuse_write(uint32_t seg_id, uint32_t data_length, uint32_t *buf)
{
    ...
    case CHIP_ID:
        if (bit_num != 128) {
            dev_err(efuse->dev,
                    "write segment %d data must length %d = 128 bit", seg_id,
                    bit_num);
            return -1;
        }
    ...
    case RANDOM_ID:
        ...
        return -1;
    case USER_ID:
    case WRITE_USER_ID_SEG1:
    case WRITE_USER_ID_SEG2_WORD0:
    case WRITE_USER_ID_SEG2_WORD1:
    case WRITE_USER_ID_SEG2_WORD2:
    case WRITE_USER_ID_SEG2_HALF_WORD3:
        if (bit_num > 128) { /* write must word align */
            dev_err(efuse->dev, "write segment %d data length %d > 128 bit",
                    seg_id, bit_num);
            return -1;
        }
        val = efuse_readl(EFUSE_STATE);
    ...
    case PROTECT_ID:
        if (bit_num > 32) { /* write must word align */
            dev_err(efuse->dev, "write segment %d data length %d > 32 bit",
                    seg_id, bit_num);
            return -1;
        }
        break;
    ...
    addr = efuse->id2addr[seg_id] - EFU_ROM_BASE;
    ...
    if (word_num > 8) {
        ...
        for (offset = 0; offset < word_num; offset++) {
            ...
            if (seg_id == WRITE_USER_ID_SEG2_HALF_WORD3) {
                val &= 0xffff;
                data_length = 1;
            }
...
static long efuse_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    struct jz_efuse
    *efuse = container_of(dev, struct jz_efuse, mdev);
    unsigned int * arg_r;
    int ret = 0;
    int i = 0;
    int times = 0;
    ...
    case CMD_READ:
        ret = jz_efuse_read(efuse->wr_info->seg_id, efuse->wr_info->data_length,
                efuse->wr_info->buf);
        break;

    case CMD_WRITE:
        if (efuse->wr_info->seg_id == USER_ID
                && efuse->wr_info->data_length > 16) {
            /* 1. write data from 0x220 to 0x230 */
            ret = jz_efuse_write(WRITE_USER_ID_SEG1, 16, efuse->wr_info->buf);
            if (ret < 0) {
                dev_err(efuse->dev, "Write user id failed\n");
                break;
            }

            /* 2. write data from 0x230 to 0x23d in word */
            times = (efuse->wr_info->data_length - 17) / 4 + 1;
            for (i = 0; i < times; i++) {
                ret = jz_efuse_write(WRITE_USER_ID_SEG2 + i, 4,
                        efuse->wr_info->buf + 4 + i);
                if (ret < 0) {
                    dev_err(efuse->dev, "Write user id failed\n");
                    break;
                }
            ...
            ret = jz_efuse_write(efuse->wr_info->seg_id,
                    efuse->wr_info->data_length, efuse->wr_info->buf);
        }
        break;
