...
extern struct platform_driver dwc2_jz_driver;
static int __init dwc2_init(void)
{
	printk("add by zxj func:%s 1\n", __func__);
	platform_driver_register(&dwc2_driver);
	mdelay(100);
	
	printk("add by zxj func:%s 2\n", __func__);
	platform_driver_register(&dwc2_jz_driver);
	return 0;
}
