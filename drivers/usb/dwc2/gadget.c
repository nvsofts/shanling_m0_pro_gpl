...
EXPORT_SYMBOL(dwc2_has_ep_enabled);
...
out_print:
	printk(dwc->dev,"enter %s:%d: plugin = %d pullup_on = %d suspend = %d\n",
		...
		int (*bind)(struct usb_gadget *))
{
	...
	printk("usb_gadget_probe_driver registering gadget driver:%s  add by zxj\n",driver->driver.name);
	if (driver == NULL) {
