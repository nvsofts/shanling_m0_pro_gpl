...
int usb_hcd_submit_urb (struct urb *urb, gfp_t mem_flags)
{
	...
	if (is_root_hub(urb->dev)) {
		...
		//printk(KERN_ERR "usb_hcd_submit_urb return status:%d ---1\n",status);
	} else {
		status = map_urb_for_dma(hcd, urb, mem_flags);
		//printk(KERN_ERR "usb_hcd_submit_urb return status:%d ---2\n",status);
		if (likely(status == 0)) {
			status = hcd->driver->urb_enqueue(hcd, urb, mem_flags);
			
			//printk(KERN_ERR "usb_hcd_submit_urb return status:%d ---2.1\n",status);
			if (unlikely(status))
	...
	if (unlikely(status)) {
		//printk(KERN_ERR "usb_hcd_submit_urb return status:%d ---3\n",status);
