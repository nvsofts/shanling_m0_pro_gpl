...
#define PWM_STABLE_TIME_US  400
#define PWM_SAMPLE_TIMES    5
#define PWM_DUTY_NS_STEP    1
...
static int get_battery_voltage(struct pwm_battery* pwm_battery) {
    ...
    for (i = 0; i < PWM_SAMPLE_TIMES; i++) {
        ...
        voltage[i] = pdata->pwm_ref_voltage * (duty_ns - PWM_DUTY_NS_STEP)
                * pdata->battery_ref_scale / pwm_battery->pwm_period;
...
static int pwm_battery_probe(struct platform_device* pdev) {
    ...
    pwm_battery->init_duty_ns = (pwm_battery->pwm_period * pwm_battery_cv_discharging[0]) /
                    (pdata->battery_ref_scale * pwm_battery->pdata->pwm_ref_voltage);
...
static int pwm_battery_resume(struct device *dev) {
    ...
    schedule_delayed_work(&pwm_battery->resume_work, 1 * HZ);
