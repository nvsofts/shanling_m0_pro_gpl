...
#include <soc/gpio.h>
int config_bluetooth_gpio(int on)
{
	if(on == 1)
	{
		jz_gpio_set_func(GPIO_PB(26),GPIO_FUNC_0);
		jz_gpio_set_func(GPIO_PC(10),GPIO_FUNC_0);
		jz_gpio_set_func(GPIO_PC(11),GPIO_FUNC_0);
		jz_gpio_set_func(GPIO_PC(12),GPIO_FUNC_0);
		jz_gpio_set_func(GPIO_PC(13),GPIO_FUNC_0);
		jz_gpio_set_func(GPIO_PC(19),GPIO_OUTPUT1);
	}
	else
	{
		jz_gpio_set_func(GPIO_PB(26),GPIO_OUTPUT0);
		jz_gpio_set_func(GPIO_PC(6),GPIO_OUTPUT0);
		jz_gpio_set_func(GPIO_PC(7),GPIO_OUTPUT0);
		jz_gpio_set_func(GPIO_PC(8),GPIO_OUTPUT0);
		jz_gpio_set_func(GPIO_PC(9),GPIO_OUTPUT0);
		jz_gpio_set_func(GPIO_PC(10),GPIO_OUTPUT0);
		jz_gpio_set_func(GPIO_PC(11),GPIO_OUTPUT0);
		jz_gpio_set_func(GPIO_PC(12),GPIO_OUTPUT0);
		jz_gpio_set_func(GPIO_PC(13),GPIO_OUTPUT0);
		jz_gpio_set_func(GPIO_PC(19),GPIO_OUTPUT0);
	}
	return 0;
}
#endif
			...
			struct class_attribute *attr,	const char *buf, size_t count)
{
	...
	if( tmp < 256 )
		...
		#ifdef CONFIG_PRODUCT_X1000_M0
		if(axp_reg_addr == 0x12)
		{
			if((val>>2)&0x1 == 1)
				config_bluetooth_gpio(1);
			else
				config_bluetooth_gpio(0);
		}
		#endif
	}
	return count;
...
int axp_charger_create_attrs(struct power_supply *psy)
{
  int j,ret;
#ifdef CONFIG_PRODUCT_X1000_M0
	config_bluetooth_gpio(0); //boot bt gpio low
#endif
