...
#define AXP_DESC_DCDC3(_family, _id, _match, _supply, _min, _max, _step, _vreg,	\
		 _vmask, _ereg, _emask) 					\
	[_family##_##_id] = {							\
		.name		= (_match),					\
		.supply_name	= (_supply),					\
...
int regulator_get_dcdc3voltage(struct regulator_dev *rdev)
{
	unsigned int val;
	int ret;
...
int regulator_set_dcdc3voltage(struct regulator_dev *rdev, unsigned sel)
{
	int ret;
...
int regulator_list_dcdc3voltage(struct regulator_dev *rdev,
				  unsigned int selector)
{
	...
	if(selector <= 70)
		ret_val = rdev->desc->min_uV + (rdev->desc->uV_step * selector);
	else if(selector > 70 && selector <= 87)
		ret_val = 1200000 + (20000*(selector-70));
	else if(selector > 87)
		ret_val = 1500000 + (100000*(selector-87));

	//printk("==fun[%s]==sel[%d]==[%d]==\n",__FUNCTION__,selector,ret_val);
	return ret_val;
}

static int axp2101_set_dcdc3voltage_time(struct regulator_dev *rdev,
		unsigned int old_selector, unsigned int new_selector)
{
	struct regulator_delay *delay = (struct regulator_delay *)rdev->reg_data;
...
static struct regulator_ops axp20x_dcdc3ops = {
	.set_voltage_sel	= regulator_set_dcdc3voltage,
	.get_voltage_sel	= regulator_get_dcdc3voltage,
	.list_voltage		= regulator_list_dcdc3voltage,
	.enable			= regulator_enable_regmap,
	.disable		= regulator_disable_regmap,
	.is_enabled		= regulator_is_enabled_regmap,
	.set_voltage_time_sel = axp2101_set_dcdc3voltage_time,
};
...
static const struct regulator_desc axp2101_regulators[] = {
	AXP_DESC(AXP2101, DCDC1, "dcdc1", "regulator-dummy", 1500, 3400, 100,
		 AXP2101_DCDC1_CFG, 0x1f, AXP2101_DCDC_CFG0, BIT(0)),

	AXP_DESC_DCDC3(AXP2101, DCDC3, "dcdc3", "regulator-dummy", 500,3400,10, //ÌØÊâ´¦Àí
	...
	AXP_DESC_FIXED(AXP2101, LDO1, "rtcldo", "regulator-dummy", 3300),
	AXP_DESC_FIXED(AXP2101, LDO2, "rtcldo1", "regulator-dummy", 3300),
	AXP_DESC(AXP2101, LDO3, "aldo1", "regulator-dummy", 500, 3500, 100,
		 AXP2101_ALDO1_CFG, 0x1f, AXP2101_LDO_EN_CFG0, BIT(0)),
	AXP_DESC(AXP2101, LDO4, "aldo2", "regulator-dummy", 500, 3500, 100,
		 AXP2101_ALDO2_CFG, 0x1f, AXP2101_LDO_EN_CFG0, BIT(1)),
	AXP_DESC(AXP2101, LDO5, "aldo3", "regulator-dummy", 500, 3500, 100,
		 AXP2101_ALDO3_CFG, 0x1f, AXP2101_LDO_EN_CFG0, BIT(2)),
	AXP_DESC(AXP2101, LDO6, "aldo4", "regulator-dummy", 500, 3500, 100,
		 AXP2101_ALDO4_CFG, 0x1f, AXP2101_LDO_EN_CFG0, BIT(3)),
	AXP_DESC(AXP2101, LDO7, "bldo1", "regulator-dummy", 500, 3500, 100,
		 AXP2101_BLDO1_CFG, 0x1f, AXP2101_LDO_EN_CFG0, BIT(4)),
	AXP_DESC(AXP2101, LDO8, "bldo2", "regulator-dummy", 500, 3500, 100,
		 AXP2101_BLDO2_CFG, 0x1f, AXP2101_LDO_EN_CFG0, BIT(5)),
	AXP_DESC(AXP2101, LDO9, "dldo1", "regulator-dummy", 500, 3500, 100,
		 AXP2101_DLDO1_CFG, 0x1f, AXP2101_LDO_EN_CFG0, BIT(7)),
	AXP_DESC(AXP2101, LDO10, "dldo2", "regulator-dummy", 500, 1400, 50,
		 AXP2101_DLDO2_CFG, 0x1f, AXP2101_LDO_EN_CFG1, BIT(0)),
	AXP_DESC(AXP2101, LDO11, "cpusldo", "regulator-dummy", 500, 1400, 50,
...
static int axp2101_regulator_probe(struct platform_device *pdev)
{
	...
	if(axp20x->variant == AXP2101_ID)
	{
		...
		for (i = 0; i < nregulators; i++) {
			...
			config.init_data = &axp_regl_init_data[i];
