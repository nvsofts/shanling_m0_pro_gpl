...
#define SPINAND_MACINFO_OFFSET  (SPINAND_UBOOT_OFFSET+SPINAND_UBOOT_SIZE)
#define SIZE_MACINFO 0x20000    /* 128k */


#define SPINAND_OMEINFO_OFFSET   (SPINAND_MACINFO_OFFSET+SIZE_MACINFO)
#define SIZE_OEMINFO 0x20000    /* 128k */


#define SPINAND_KERNEL_OFFSET   (SPINAND_OMEINFO_OFFSET + SIZE_OEMINFO)
#define SPINAND_KERNEL_SIZE 0x600000

#ifdef CONFIG_SUPPORT_OTA
#define SPINAND_RECOVERY_OFFSET   (SPINAND_KERNEL_OFFSET + SPINAND_KERNEL_SIZE)
#define SIZE_RECOVERY 0x800000

#define SPINAND_ROOTFS_OFFSET   (SPINAND_RECOVERY_OFFSET + SIZE_RECOVERY)
#define SPINAND_ROOTFS_SIZE 	0x5000000	
#else
...
struct jz_sfcnand_partition spinand_partition[] = {
	...
	{
		.name =    "macinfo",
		.offset =  SPINAND_MACINFO_OFFSET,
		.size =    SIZE_MACINFO,
		.mask_flags = SPINAND_MASK_FLAGS,
	},
	{
		.name =    "omeinfo",
		.offset =  SPINAND_OMEINFO_OFFSET,
		.size =    SIZE_OEMINFO,
		.mask_flags = SPINAND_MASK_FLAGS,
	},
	...
	{
		.name	=		"recovery",
		.offset =	SPINAND_RECOVERY_OFFSET,
		.size	=	SIZE_RECOVERY,
		.mask_flags = SPINAND_MASK_FLAGS,
	},
#endif
