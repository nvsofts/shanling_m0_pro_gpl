...
{	
	#define GPIO_TF_EN        GPIO_PB(12)
	#define GPIO_USB_ID            	GPIO_PC(23)
	#define GPIO_USB_DETE           GPIO_PA(17)
	#define GPIO_PO_DETE            GPIO_PB(6)
	#define GPIO_BAL_DETE           GPIO_PB(7)
	#define GPIO_UART0_CTS          GPIO_PC(12)
	#define GPIO_OTG_CONTROL			GPIO_PC(3)

	
    gpio_request(GPIO_TF_EN, "tf_en");
    gpio_direction_output(GPIO_TF_EN, 1);

	gpio_request(GPIO_USB_DETE,"usb_detect");
	gpio_direction_input(GPIO_USB_DETE);
	
	gpio_request(GPIO_USB_ID,"otg_id");
	gpio_direction_input(GPIO_USB_ID);

	gpio_request(GPIO_PO_DETE,"po_detect");
	gpio_direction_input(GPIO_PO_DETE);

	gpio_request(GPIO_BAL_DETE,"bal_detect");
	gpio_direction_input(GPIO_BAL_DETE);

	gpio_request(GPIO_OTG_CONTROL,"otg control");
	gpio_direction_output(GPIO_OTG_CONTROL,1); 


	gpio_request(GPIO_UART0_CTS,"uart0_cts");
	gpio_direction_output(GPIO_UART0_CTS,0);

	#if 0
	{
		int ret,ret1;
		ret = gpio_request(GPIO_PC(10),"uart0_tx");
		gpio_direction_input(GPIO_PC(10));
		ret1 = gpio_request(GPIO_PC(11),"uart0_rx");
		gpio_direction_input(GPIO_PC(11));	
	}
	#endif
	...
	//将otg-powerodwn,会少4~5mA的电
	jz_otg_phy_powerdown();
}
#endif

#ifdef CONFIG_PRODUCT_X1000_HALLEY2
void gpio_power_init(void)
{	

	#define GPIO_LCD_EN        GPIO_PC(24)
	#define GPIO_TF_EN        GPIO_PC(23)
	#define GPIO_CODEC_EN     GPIO_PB(6)
	#define DACOUT_POWER	  GPIO_PB(24)
	#define GPIO_PWM_EN        GPIO_PC(25)
	
	#define GPIO_RELAY_EN        GPIO_PC(16)

	#define GPIO_MUTE_EN		 GPIO_PD(4)

	
    
    gpio_request(GPIO_TF_EN, "tf_en");
	gpio_direction_output(GPIO_TF_EN, 0);
	udelay(100);
    gpio_direction_output(GPIO_TF_EN, 1);
    gpio_request(GPIO_LCD_EN, "lcd_en");
    gpio_direction_output(GPIO_LCD_EN, 1);
    gpio_request(GPIO_PWM_EN, "pwm_en");
    gpio_direction_output(GPIO_PWM_EN, 1);
    
    gpio_request(GPIO_CODEC_EN, "codec_en");
    gpio_direction_output(GPIO_CODEC_EN, 1);

	gpio_request(DACOUT_POWER, "dacout_power");
    gpio_direction_output(DACOUT_POWER, 0);
	
	gpio_request(GPIO_RELAY_EN, "relay_en");
    gpio_direction_output(GPIO_RELAY_EN, 0);
	gpio_request(GPIO_MUTE_EN, "mute_en");
    gpio_direction_output(GPIO_MUTE_EN, 0);
	
    
    printk(KERN_ERR "GPIO_TF_EN set low add by zxj\n");
	printk(KERN_ERR "GPIO_LCD_EN set hight add by zxj\n");
	printk(KERN_ERR "GPIO_CODEC_EN set hight add by zxj\n");	
	printk(KERN_ERR "DACOUT_POWER set low add by zxj\n");
	printk(KERN_ERR "relay_en set low add by zxj\n");

	#define GPIO_PB_12 GPIO_PB(12)
	#define GPIO_PB_13 GPIO_PB(13)
	#define GPIO_PB_14 GPIO_PB(14)
	#define GPIO_PB_15 GPIO_PB(15)
	gpio_request(GPIO_PB_12, "GPIO_PB_12");
	gpio_direction_input(GPIO_PB_12);
	
	gpio_request(GPIO_PB_13, "GPIO_PB_13");
	gpio_direction_input(GPIO_PB_13);
	
	gpio_request(GPIO_PB_14, "GPIO_PB_14");
	gpio_direction_input(GPIO_PB_14);
	
	gpio_request(GPIO_PB_15, "GPIO_PB_15");
	gpio_direction_input(GPIO_PB_15);
...
ssize_t pin_read_proc (struct seq_file *m, void *v)
{
	int len = 0;
	len += seq_printf(m, "pin = %d\n",gpio_pin);
	return len;
}

ssize_t pin_write_proc (struct file *file, const char __user *buf, size_t count, loff_t *off)
{	
	if(count > 0 && count <= 60)
	{
   	 	char tmp_pin[64];  
   	 	if(copy_from_user(tmp_pin, buf, count))  
    	{   
        	return EFAULT;  
    	}  
	 	gpio_pin = simple_strtoul(tmp_pin, 0, 10);
	}
	return count;
}


ssize_t gpio_read_proc (struct seq_file *m, void *v)
{
	int len = 0;
	if(gpio_pin >= 0)
		len += seq_printf(m, "pin = %d,status = %d\n",gpio_pin,gpio_get_value(gpio_pin));
	else
		len += seq_printf(m, "pin = -1,status = 0\n");
	return len;
}

ssize_t gpio_write_proc (struct file *file, const char __user *buf, size_t count, loff_t *off)
{	
	if(count > 0 && count <= 60)
	{
		char tmp_status[64];
		int n_status;
	    if(copy_from_user(tmp_status, buf, count))  
   	 	{   
       	 	return EFAULT;  
    	}  
	 	n_status = simple_strtoul(tmp_status, 0, 10);
...
static int gpio_pin_read_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, pin_read_proc, PDE_DATA(inode));
}

static int gpio_status_read_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, gpio_read_proc, PDE_DATA(inode));
}
...
static const struct file_operations gpio_pin_fops ={
	.owner   = THIS_MODULE, 
	.read = seq_read,
	.write = pin_write_proc,
	.open = gpio_pin_read_proc_open,
	.llseek = seq_lseek,
	.release = single_release,
};

static const struct file_operations gpio_status_fops ={
	.owner	 = THIS_MODULE, 

	.read = seq_read,
	.write = gpio_write_proc,
	.open = gpio_status_read_proc_open,
	.llseek = seq_lseek,
	.release = single_release,
};
//***********************************
static int __init init_gpio_proc(void)
{
	...
	//*****cyadd****pin*********	
	proc_create("pin", 0600,p,&gpio_pin_fops);
	res = proc_create("status", 0600,p,&gpio_status_fops);
	//**************************
	#if 1
  	gpio_power_init();
    #endif
